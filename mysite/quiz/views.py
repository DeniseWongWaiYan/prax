...
def QuizListView(request):
    ...
    results = Sitting.objects.filter(complete=True, user=request.user).order_by('-end')      
    percentages = [result.get_percent_correct for result in results]
...
class QuizMarkingList(QuizMarkerMixin, SittingFilterTitleMixin, ListView):
    ...
    def get_queryset(self):
        queryset = super(QuizMarkingList, self).get_queryset().filter(complete=True).order_by('end')                                         
